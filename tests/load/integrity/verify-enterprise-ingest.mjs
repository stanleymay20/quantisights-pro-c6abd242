...
if (jobs.length === 0) {
  console.error(`NO GO: no data_sync_jobs found for LOAD_RUN_ID=${runId}`);
  process.exit(1);
}
...
const persistedRows = Number(contentRange.split("/")[1] || 0);
...
console.log("GO: queued enterprise ingestion drained completely with matching persisted-row evidence");
