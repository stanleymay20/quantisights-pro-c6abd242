  ...
  return (
    <div className="qv-mock-frame qv-ledger">
      ...
      <div style={{ opacity: fade ? 0.35 : 1, transition: "opacity 0.35s" }}>
        {visible.map(index => { const d = DECISIONS[index]; return <div className="qv-ledger-row" key={d.id}><span style={{ color: "rgba(255,255,255,0.5)", fontSize: 10 }}>{d.id}</span><span style={{ color: "#fff", fontSize: 12 }}>{d.category}</span><span style={{ color: "#fff", fontSize: 12 }}>{d.confidence}%</span><span style={{ color: "#fff", fontSize: 12 }}>{d.impact}</span><span style={{ color: "#fff", fontSize: 11 }}>{d.tag}</span><span style={{ color: "rgba(255,255,255,0.6)", fontSize: 11 }}>{d.time}</span><span style={{ color: "#4ADE80", fontSize: 11 }}>{d.governance}</span></div>; })}
      </div>
      <div className="qv-mobile-card"><div style={{ fontSize: 10, color: "rgba(255,255,255,0.55)" }}>{primaryDecision.id}</div><div className="qv-mobile-card-title">{primaryDecision.category}</div><div className="qv-mobile-metrics"><div className="qv-mobile-metric"><div style={{ color: "#fff", fontWeight: 700 }}>{primaryDecision.confidence}%</div><div style={{ fontSize: 9, color: "rgba(255,255,255,0.55)" }}>CONF.</div></div><div className="qv-mobile-metric"><div style={{ color: "#fff", fontWeight: 700 }}>{primaryDecision.impact}</div><div style={{ fontSize: 9, color: "rgba(255,255,255,0.55)" }}>IMPACT</div></div><div className="qv-mobile-metric"><div style={{ color: "#4ADE80", fontWeight: 700 }}>{primaryDecision.governance}</div><div style={{ fontSize: 9, color: "rgba(255,255,255,0.55)" }}>GOV.</div></div></div></div>
    </div>
  );
};
  ...
  <section style={{ background: DEEP, paddingBottom: 96 }}>
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px" }}>
      <div className="qv-grid-2">
        <div><Eyebrow tone="light" style={{ display: "block", marginBottom: 20 }}>The governance record</Eyebrow><h2 style={{ fontFamily: "Georgia, serif", fontSize: "clamp(26px, 3vw, 42px)", lineHeight: 1.15, letterSpacing: "-0.02em", color: "#fff", fontWeight: 400, margin: "0 0 24px" }}>Every decision. Full evidence chain. One auditable record.</h2><p style={{ fontSize: 15, color: "rgba(255,255,255,0.78)", lineHeight: 1.75, margin: "0 0 32px" }}>The Decision Ledger captures AI recommendations, human approvals, supporting evidence, predicted outcomes and observed outcomes so teams can review how consequential decisions were made.</p>{[["Recommendation → Decision → Outcome → Learning", "The full loop, recorded in one governed workflow"], ["Source-aware outputs on governed recommendations", "Evidence references are carried into the decision record for review"], ["Calibration after measurable outcomes", "Prediction performance can be compared with observed results over time"]].map(([title, sub]) => <div key={title} style={{ display: "flex", gap: 14, alignItems: "flex-start", marginBottom: 16 }}><div style={{ width: 20, height: 20, borderRadius: "50%", background: "rgba(34,197,94,0.18)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2 }}><span style={{ color: "#4ADE80", fontSize: 11, fontWeight: 700 }}>✓</span></div><div><div style={{ fontSize: 13, fontWeight: 600, color: "#fff", marginBottom: 2 }}>{title}</div><div style={{ fontSize: 12, color: "rgba(255,255,255,0.65)" }}>{sub}</div></div></div>)}</div>
        <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 8, overflow: "hidden" }}><div style={{ padding: "20px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}><div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}><TagBadge tone="Warning" style={{ borderRadius: 2 }}>Risk Mitigation</TagBadge><TagBadge tone="Success" style={{ borderRadius: 2 }}>Governance Active</TagBadge></div><div style={{ fontSize: 13, color: "#fff", fontWeight: 500, lineHeight: 1.4 }}>Illustrative inventory and working-capital decision</div><div style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", marginTop: 6 }}>Example record · Source: enterprise connector</div></div><div className="qv-form-grid">{[["90%", "Illustrative confidence", "#22C55E"], ["+€20K", "Illustrative impact", "#fff"], ["Strong", "Evidence", "#F59E0B"]].map(([value, label, color]) => <div key={label} style={{ background: "rgba(255,255,255,0.03)", padding: "14px 16px" }}><div style={{ fontSize: 20, fontWeight: 700, color }}>{value}</div><div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em" }}>{label}</div></div>)}</div><div style={{ padding: "14px 20px" }}>{["Source reference attached", "Governance checks recorded", "Human approval captured"].map(item => <div key={item} style={{ display: "flex", gap: 10, marginBottom: 8, alignItems: "center" }}><div style={{ width: 6, height: 6, borderRadius: "50%", background: "#22C55E" }} /><span style={{ fontSize: 11, color: "rgba(255,255,255,0.78)" }}>{item}</span></div>)}</div></div>
      </div>
    </div>
  </section>
);
...
const SecurityTrust = () => <section style={{ background: MUTED, borderTop: `1px solid rgba(30,39,97,0.1)`, borderBottom: `1px solid rgba(30,39,97,0.1)` }}><div style={{ maxWidth: 1280, margin: "0 auto", padding: "64px 24px" }}><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "start" }}><div><Eyebrow style={{ marginBottom: 16 }}>Security & assurance</Eyebrow><h2 style={{ fontFamily: "Georgia, serif", fontSize: "clamp(22px, 2.5vw, 34px)", color: NAVY, fontWeight: 400, margin: "0 0 24px", lineHeight: 1.2 }}>Built for procurement teams, not just developers.</h2><div style={{ display: "flex", gap: 8, marginBottom: 24, flexWrap: "wrap" }}>{[{ label: "SOC 2 Type II", status: "Not claimed without report" }, { label: "ISO 27001", status: "Not claimed without certificate" }, { label: "TISAX", status: "Not claimed without assessment" }, { label: "BSI C5", status: "Not claimed without attestation" }].map(({ label, status }) => <div key={label} style={{ border: "1px solid rgba(30,39,97,0.18)", borderRadius: 6, padding: "8px 12px" }}><div style={{ fontSize: 12, fontWeight: 700, color: NAVY }}>{label}</div><div style={{ fontSize: 11, color: SLATE, marginTop: 2 }}>{status}</div></div>)}</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>{[["Region disclosure", REGION_DISCLOSURE_SHORT], ["GDPR-oriented controls", "DPA support, privacy controls and data-governance workflows are available; customers remain responsible for their own compliance obligations."], ["EU AI Act support", "Governance records can support risk-management, transparency and human-oversight workflows where those obligations apply."], ["Encryption", "Encryption controls protect data at rest and in transit; deployment-specific details are documented in the Trust Center."], ["Access Control", "SAML SSO, SCIM provisioning, MFA, WebAuthn passkeys and RBAC are available according to plan and configuration."], ["Audit Trail", "Tamper-evident decision records and evidence verification support later review."]].map(([label, desc]) => <div key={label} style={{ padding: "14px 0", borderBottom: `1px solid rgba(30,39,97,0.08)` }}><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}><span style={{ color: "#16a34a", fontSize: 12, fontWeight: 700 }}>✓</span><span style={{ fontSize: 12, fontWeight: 600, color: NAVY }}>{label}</span></div><p style={{ fontSize: 11, color: SLATE, lineHeight: 1.5, margin: 0, paddingLeft: 20 }}>{desc}</p></div>)}</div><div style={{ marginTop: 24, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}><a href="/trust" style={{ fontSize: 12, color: ACCENT, textDecoration: "none", display: "flex", alignItems: "center", gap: 4, fontWeight: 500 }}>View Trust Center →</a><a href="/procurement-pack" style={{ fontSize: 12, color: SLATE, textDecoration: "none", display: "flex", alignItems: "center", gap: 4 }}>View procurement pack →</a></div></div><div><Eyebrow style={{ marginBottom: 16 }}>Why not just use Microsoft?</Eyebrow><h2 style={{ fontFamily: "Georgia, serif", fontSize: "clamp(22px, 2.5vw, 34px)", color: NAVY, fontWeight: 400, margin: "0 0 24px", lineHeight: 1.2 }}>Different layers of the governance stack.</h2><p style={{ fontSize: 15, color: SLATE, lineHeight: 1.75, margin: "0 0 24px" }}>Quantivis is designed around the business decision record: who approved an action, what evidence supported it, what outcome was expected and what happened afterwards. Existing productivity, compliance and workflow platforms can remain part of the surrounding stack.</p><div style={{ display: "flex", flexDirection: "column", gap: 12 }}>{[["AI productivity tools", "Assist users with AI-generated work", "Quantivis", "Focuses on governed business decisions and outcomes"], ["Data compliance platforms", "Govern documents, identities and data", "Quantivis", "Adds a decision-level evidence and approval record"], ["Workflow platforms", "Automate operational processes", "Quantivis", "Adds decision governance and outcome learning"]].map(([comp, compDesc, qv, qvDesc]) => <div key={comp} style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", gap: 8, alignItems: "center", padding: "12px 0", borderBottom: `1px solid rgba(30,39,97,0.08)` }}><div><div style={{ fontSize: 12, fontWeight: 600, color: NAVY }}>{comp}</div><div style={{ fontSize: 12, color: SLATE, lineHeight: 1.45 }}>{compDesc}</div></div><div style={{ fontSize: 11, color: "hsl(var(--brand-executive-navy) / 0.333)", fontStyle: "italic" }}>and</div><div><div style={{ fontSize: 12, color: NAVY, fontWeight: 700 }}>{qv}</div><div style={{ fontSize: 12, color: SLATE, lineHeight: 1.45 }}>{qvDesc}</div></div></div>)}</div></div></div></div></section>;
...
const Pricing = () => {
  ...
  return <section id="pricing" style={{ background: "#fff" }}><div className="qv-wrap"><Eyebrow style={{ marginBottom: 24 }}>Pricing</Eyebrow><div className="qv-grid-2" style={{ gap: 48, marginBottom: 16 }}><h2 className="qv-heading">Straightforward pricing. No hidden tier surprises.</h2><p style={{ fontSize: 15, color: SLATE, lineHeight: 1.75, margin: 0 }}>Evaluate Quantivis with a {PILOT_TERMS.days}-day no-card {PILOT_TERMS.tierLabel} pilot. Eligible paid checkouts may also include a {COMMERCIAL_TERMS.trialDays}-day checkout trial; billing and renewal terms are shown before confirmation.</p></div><p style={{ fontSize: 12, color: SLATE, margin: "0 0 32px", letterSpacing: "0.02em" }}>Current plan capabilities below come from the same commercial tier definitions used by the application. All amounts exclude VAT.</p><div className="qv-grid-3" style={{ background: `rgba(30,39,97,0.08)` }}>{plans.map(plan => <div key={plan.name} className="qv-card" style={{ display: "flex", flexDirection: "column", position: "relative", borderRadius: 0, outline: plan.featured ? `2px solid ${ACCENT}` : "none", outlineOffset: -2 }}>{plan.featured && <div style={{ position: "absolute", top: 0, left: 0, right: 0, background: ACCENT, textAlign: "center", fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#fff", padding: "6px 0" }}>Recommended for governed teams</div>}<div style={{ paddingTop: plan.featured ? 24 : 0 }}><h3 style={{ fontFamily: "Georgia, serif", fontSize: 22, color: NAVY, fontWeight: 400, margin: "0 0 20px" }}>{plan.name}</h3><div><span style={{ fontFamily: "Georgia, serif", fontSize: 40, color: NAVY }}>{plan.price}</span><span style={{ fontSize: 13, color: SLATE }}> {plan.cadence}</span></div><div style={{ fontSize: 13, color: SLATE, marginBottom: 24 }}>{plan.seats}</div>{plan.features.map(feature => <div key={feature} style={{ display: "flex", gap: 10, marginBottom: 12 }}><span style={{ color: "#16a34a", fontWeight: 700 }} aria-hidden="true">✓</span><span style={{ fontSize: 13, color: "hsl(var(--brand-executive-navy) / 0.8)", lineHeight: 1.5 }}>{feature}</span></div>)}</div></div>)}</div></div></section>;

const Demo = forwardRef<HTMLElement>((_, ref) => <section id="demo" ref={ref} style={{ background: DEEP, color: "#fff" }}><div className="qv-wrap"><div className="qv-grid-2"><div><Eyebrow tone="light" style={{ marginBottom: 24 }}>See Quantivis in your workflow</Eyebrow><h2 style={{ fontFamily: "Georgia, serif", fontSize: "clamp(30px, 4vw, 52px)", fontWeight: 400, margin: "0 0 24px", lineHeight: 1.15 }}>Bring one consequential decision. We’ll show you the evidence trail.</h2><p style={{ color: "rgba(255,255,255,0.7)", lineHeight: 1.7, fontSize: 15 }}>A strong pilot starts with one measurable decision process, the systems that inform it, the required approval path and the outcome you want to track.</p></div><div><Link to="/contact" className="qv-primary-cta">Request a working session <ArrowRight size={16} /></Link><p style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", marginTop: 16 }}>No fabricated ROI claims. We define pilot success criteria with you before measuring results.</p></div></div></div></section>);
