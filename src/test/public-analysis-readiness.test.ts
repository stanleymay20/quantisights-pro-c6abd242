...
describe("release provenance and CSP release gates", () => {
  it("keeps dependency installation portable outside the Lovable environment", () => {
    ...
    // npm is the release-install path. A bun lockfile may be regenerated by
    // local tooling, but it must never be committed or checked in: the gate is
    // that it stays untracked (git-ignored), not merely absent from disk.
    expect(execFileSync("git", ["ls-files", "bun.lock", "bun.lockb"], { cwd: root, encoding: "utf8" }).trim()).toBe("");
    expect(read(".gitignore")).toContain("bun.lock");
