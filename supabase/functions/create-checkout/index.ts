...
import {
  getSelfServeCatalogEntry,
  isBillingInterval,
  isSelfServeBillingTier,
...
serve(async (req) => {
  ...
  try {
    ...
    if (!isSelfServeBillingTier(requestedTier) || !isBillingInterval(requestedInterval)) {
      return new Response(JSON.stringify({ error: "Unsupported Quantivis billing selection" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 400,
      });
    }
    const catalogEntry = getSelfServeCatalogEntry(requestedTier, requestedInterval);
